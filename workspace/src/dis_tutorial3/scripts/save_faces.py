...
class Face():
    ...
    def __init__(self, id, center_point, bottom_right_point, upper_left_point, current_time, robot_position, count):
        self.id = id
...
class PeopleMarkerSubscriber(Node):
    ...
    def marker_callback(self, msg):
        ...
        current_bottom_right_position = np.array([msg.bottom_right.pose.position.x, msg.bottom_right.pose.position.y, msg.bottom_right.pose.position.z])
        current_upper_left_position = np.array([msg.upper_left.pose.position.x, msg.upper_left.pose.position.y, msg.upper_left.pose.position.z])

        print(f"Center: {current_position}, Bottom left: {current_bottom_right_position}, Upper right: {current_upper_left_position}")
        ...
        try:
            ...
            print(f"Transformed center: {transformed_position}, Transformed bottom left: {transformed_bottom_right_position}, Transformed upper right: {transformed_upper_left_position}")
            time.sleep(1)
        ...
        self.faces[self.face_counter] = Face(self.face_counter, transformed_position, transformed_bottom_right_position, transformed_upper_left_position, current_time, self.robot_position, 1)
