...
from task_2s.srv import BirdCollection  # Adjust package name if needed
from task_2s.msg import Bird  # This must be a msg, since Bird[] is used in the service

from sensor_msgs.msg import Image
from cv_bridge import CvBridge
import cv2

class BirdCatalogueClient(Node):
    def __init__(self):
        super().__init__('bird_catalogue_client')
        self.cli = self.create_client(BirdCollection, 'bird_catalogue')

        while not self.cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Waiting for bird_catalogue service...')
