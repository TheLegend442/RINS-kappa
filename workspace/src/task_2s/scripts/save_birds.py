...
class BirdMarkerSubscriber(Node):
    def __init__(self):
        ...
        self.time_threshold = 0.01  # Sekunde preden obraz ponovno upoštevamo
        self.detections_needed = 6
    ...
    def process_marker(self, msg):
        ...
        new_bird = Bird(self.bird_counter, transformed_position, self.robot_position, count=1)
        self.birds[self.bird_counter] = new_bird
        if new_bird.count >= self.detections_needed:
            # **Objavimo nov marker**
            self.publish_bird_marker(transformed_position, self.bird_counter)
