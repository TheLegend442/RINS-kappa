...
class detect_rings(Node):
    ...
    def image_callback(self,data):
        ...
        for ring in self.rings:
            ring_msg = self.create_ring_coordinates_message(ring, ros_time)
            self.marker_pub.publish(ring_msg)
    ...
    def create_marker(self, d, ros_time):
        marker = Marker()
    ...
    def intrinsic_tf(self, cx, cy, stamp):
        ...
        try:
            ...
            transform = self.tf_buffer.lookup_transform('base_link', 'oakd_rgb_camera_optical_frame', rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=0.3))
    ...
    def create_ring_coordinates_message(self, ring, ros_time):
        ...
        for y in range(min_y, max_y):
            for x in range(min_x, max_x):
                ...
                if val1 < 1:
                    ...
                    if val2 > 1:
                        d = self.intrinsic_tf(x,y,rclpy.time.Time())
                        if not d is None and not np.isnan(d[0]) and not np.isinf(d[0]):
        ...
        ring_coordinates.center = self.create_marker(coordinate_sum, ros_time)
